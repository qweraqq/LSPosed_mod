...
void Logcat::ProcessBuffer(struct log_msg *buf) {
    ...
    bool shortcut = false;
    if (tag == "LSPsd-Bridge"sv || tag == "XSharedPreferences"sv || tag == "LSPsdContext") [[unlikely]] {
    ...
    if (verbose_ && (shortcut || buf->id() == log_id::LOG_ID_CRASH ||
                     ...
                     tag == "LSPlant"sv || tag.starts_with("LSPlog"sv))) [[unlikely]] {
        verbose_print_count_ += PrintLogLine(entry, verbose_file_.get());
    }
    if (entry.pid == my_pid_ && tag == "LSPsdLogcat"sv) [[unlikely]] {
